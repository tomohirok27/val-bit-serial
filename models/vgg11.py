...
class Vgg(nn.Module):
    ...
    def forward(self, x):
        # １層目
        x = self.maxpool(self.relu(self.bn_conv1_1(self.conv1_1(x))))

        # ２層目
        x = self.maxpool(self.relu(self.bn_conv2_1(self.conv2_1(x))))

        # ３層目
        x = self.relu(self.bn_conv3_1(self.conv3_1(x)))
        x = self.maxpool(self.relu(self.bn_conv3_2(self.conv3_2(x))))

        # ４層目
        x = self.relu(self.bn_conv4_1(self.conv4_1(x)))
        x = self.maxpool(self.relu(self.bn_conv4_2(self.conv4_2(x))))

        # ５層目
        x = self.relu(self.bn_conv5_1(self.conv5_1(x)))
        x = self.maxpool(self.relu(self.bn_conv5_2(self.conv5_2(x))))
...
def _vgg11(quant,num_classes, fixed_num,
