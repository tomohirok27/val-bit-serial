...
from dataset.cifer import dataset_loader
...
def evaluation():
            ...
            for i in range(len(th_list)):
                ...
                for j in range(len(output_Nbit)):
                    if outputs[j].abs().max() == 0.:
                        E = entropy(softmax(output_Nbit[j]))
    ...
    print(sum_bit_len / len(test_loader.dataset))
...
if __name__ == '__main__':
    ...
    _, test_loader = dataset_loader(args.dataset, test_batch=args.batch_size)
